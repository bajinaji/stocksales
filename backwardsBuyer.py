...
def processStocks():
    ...
    for i in range(len(stock_price)-1 - min_hold, -1, -1):
        ...
        # set values if traded stock at position i at all possible times from min hold time to max hold time + best choice in next time iteration
        # to account for total profit at given time step for given sell time
        ...
        # set path taken so far for maximum result
        ...
        # I like having the direct path accessible
        best_action[i] = max_index[0]
